...
const Turno = () => {
  ...
  const [form, setForm] = useState({ proyecto_id: '', nombre: '', descripcion: '', activo: true })
  ...
  const openCreate = () => {
    setEditing(null)
    setEditingFull(null)
    setForm({ proyecto_id: selectedProyectoId || '', nombre: '', descripcion: '', activo: true })
    setModalOpen(true)
  }

  const openEdit = async (t) => {
    setEditing(t)
    setForm({ proyecto_id: t.proyecto_id, nombre: t.nombre, descripcion: t.descripcion || '', activo: t.activo ?? true })
    setModalOpen(true)
    try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await turnosService.update(editing.turno_id, { nombre: form.nombre, descripcion: form.descripcion, activo: form.activo })
      } else {
        await turnosService.create(form)
      }
      setModalOpen(false)
      loadData()
    } catch (err) {
