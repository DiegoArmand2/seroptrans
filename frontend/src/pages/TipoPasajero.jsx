...
const TipoPasajero = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [tRes, pRes] = await Promise.all([tiposPasajeroService.list(params), proyectosService.list()])
      setTipos(tRes.data)
  ...
  const openEdit = async (t) => {
    ...
    setModalOpen(true)
    try {
      const { data } = await tiposPasajeroService.get(t.tipo_pasajero_id)
      setEditingFull(data)
    } catch {
      setEditingFull(t)
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editing) {
        await tiposPasajeroService.update(editing.tipo_pasajero_id, {
          codigo: form.codigo,
          nombre: form.nombre,
        ...
        await tiposPasajeroService.create(form)
      }
      setModalOpen(false)
      loadData()
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (!confirm('¿Eliminar este tipo de pasajero?')) return
    try {
      await tiposPasajeroService.delete(id)
      loadData()
    } catch (err) {
  ...
  return (
    <>
      <PageHeader
        ...
        description="Catálogo de tipos de pasajero por proyecto"
        children={
              ...
              {tipos.length === 0 && (
                <tr>
                  <td colSpan={6} className="py-12 text-center text-muted">
                    {selectedProyectoId ? 'No hay tipos en este proyecto' : 'No hay tipos de pasajero'}
                  </td>
                </tr>
              )}
              {tipos.map((t) => (
                <tr key={t.tipo_pasajero_id} className="border-b border-primary/5 hover:bg-primary/5">
                  <td className="py-3 px-4">{proyectos.find((p) => p.proyecto_id === t.proyecto_id)?.nombre || '-'}</td>
                  <td className="py-3 px-4 font-mono text-sm">{t.codigo}</td>
                  ...
                  <td className="py-3 px-4 text-right">
                    <div className="flex justify-end gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => handleDelete(t.tipo_pasajero_id)}
                      />
                    </div>
                  </td>
                </tr>
      ...
      <Modal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Editar tipo de pasajero' : 'Nuevo tipo de pasajero'}>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <label htmlFor="activo-tp" className="text-sm font-medium text-primary">
              Activo
            </label>
          </div>
